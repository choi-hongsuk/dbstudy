...
/*
    트랜잭션
    1. 한 번에 수행해야하는 작업을의미한다. (원자성, ALL or Nothing)
...
CREATE TABLE EMPLOYEE_T (
    ...
    HIRE_DATE   DATE            ,
    SALARY      NUMBER          ,     
    CONSTRAINT PK_EMPLOTEE PRIMARY KEY(EMP_NO),
    CONSTRAINT FK_EMPLOTEE_DEPATMENT FOREIGN KEY(DEPART) REFERENCES DEPARTMENT_T(DEPT_NO) ON DELETE SET NULL
    );
...
-- 2. EMPLOYEE_T에서 부서번호(DEPART)가 2인 부서의 사원들의 기본급(SALARY)을 10% 인상해주시오.
UPDATE EMPLOYEE_T
...
-- 2. EMPLOYEE_T에서 부서번호가(DEPART)가 1과 4인 사원을 삭제하시오. 
DELETE 
  FROM EMPLOYEE_T 
 WHERE DEPART IN (1, 4);

-- 3. DEPARTMENT_T에서 부서번호가(DEPT_NO)가 2인 부서를 삭제하시오. (부서번호가 2인 사원이 있다. 부서가 없어지면 사원 정보가 참조 무결성에 위배된다. 참조 무결성 위배에 대비해서 ON DELETE SET NULL 옵션을 주었으므로 사원 테이블의 부서번호가 NULL로 함께 수정된다.)
DELETE
  FROM DEPARTMENT_T
 WHERE DEPT_NO = 2;