...
SELECT *
  FROM DEPARTMENT_T
 WHERE LOCATION = '서울'; -- LOCATION 칼럼은 UNIQUE하지 않기 때문에 조회 결과가 2개 이상일 수 있다.
 
-- 6. 사원 테이브렝서 기본급이 300만 이상인 사원을 조회하시오.
SELECT *
  FROM EMPLOYEE_T
 WHERE SALARY >= 3000000;
 
-- 7. 사원 테이블에서 기본급이 200만에서 300만 사이인 사원을 조회하시오.
SELECT *
  FROM EMPLOYEE_T
