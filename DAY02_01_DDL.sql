...
/*
    테이블 수정하기
    1. 칼럼 추가하기          >> ALTER TABLE 테이블 명 ADD           칼럴명 데이터타입 제약조건
    2. 칼럼 수정하기          >> ALTER TABLE 테이블 명 MODIFY        칼럼명 데이터타입 제약조건
    3. 칼럼 삭제하기          >> ALTER TABLE 테이블 명 DROP COLUMN   칼럼명
    4. 칼럼 이름 바꾸기       >> ALTER TABLE 테이블 명 RENAME COLUMN 기존칼럼명 TO 신규칼럼명
    5. 테이블 이름 바꾸기     >> ALTER TABLE 테이블 명 RENAME TO     신규테이블명
*/

-- 1. BANK_T에 연락처(BANK_TEL)칼럼을 추가하시오.
ALTER TABLE BANK_T ADD BANK_TEL VARCHAR2(20) NULL;
-- 2. BANK_T에 은행명(BANK_NAME)칼럼을 VARCHAR2(15)로 바꾸고 필수 제약 조건을 지정하시오.
...
ALTER TABLE CUSTOMER_T ADD GRADE VARCHAR2(6) CHECK(GRADE = 'VIP' OR GRADE = 'GOLD' OR GRADE = 'SILVER' OR GRADE = 'BRONZE');
...
테이블 수정하기 - PK / FK 제약조건
    ...
    2. FK
        1) 추가     : ALTER TABLE 자식테이블명 ADD CONSTRAINT 제약조건명 FORIEIGN KEY(칼럼명) REFERENCES 부모테이블명(참조할 칼럼명) [옵션]
...
-- FK_CUSTOMER_BANK 제약조건을 활성화 하시오.
