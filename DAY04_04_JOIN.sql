...
SELECT E.EMPLOYEE_ID, CONCAT(CONCAT(E.FIRST_NAME, ' '), E.LAST_NAME), D.DEPARTMENT_NAME
  FROM DEPARTMENTS D INNER JOIN EMPLOYEES E          -- PK 키를 왼쪽으로 FK 키를 오른쪽으로
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
    
-- 2. 부서번호, 부서명, 지역명을 조회하시오.
SELECT D.DEPARTMENT_ID, D.DEPARTMENT_NAME, L.STREET_ADDRESS
  FROM LOCATIONS L INNER JOIN DEPARTMENTS D          -- PK 키를 왼쪽으로 FK 키를 오른쪽으로
    ON L.LOCATION_ID = D.LOCATION_ID;
    
-- 3. 사원번호, 사원명, 직업, 직업별 최대연봉, 연봉을 조회하시오.
SELECT E.EMPLOYEE_ID, E.FIRST_NAME, E.JOB_ID, J.MAX_SALARY, E.SALARY
  FROM JOBS J INNER JOIN EMPLOYEES E                 -- PK 키를 왼쪽으로 FK 키를 오른쪽으로
    ON J.JOB_ID = E.JOB_ID;              
    
-- 4. 사원번호, 사원명, 부서명을 조회하시오. 부서가 없으면 부서 없음으로 조회하시오.
SELECT E.EMPLOYEE_ID, E.FIRST_NAME, NVL(D.DEPARTMENT_NAME, '부서 없음')
  FROM DEPARTMENTS D RIGHT JOIN EMPLOYEES E          -- PK 키를 왼쪽으로 FK 키를 오른쪽으로
    ON D.DEPARTMENT_ID = E.DEPARTMENT_ID;
...
-- 3개 이상 테이블 조인하기
-- 6. 사원번호, 사원명, 부서번호, 부서명, 지역번호, 직역명을 조회하시오.
SELECT E.EMPLOYEE_ID, CONCAT(CONCAT(E.FIRST_NAME, ' '), E.LAST_NAME), D.DEPARTMENT_ID, D.DEPARTMENT_NAME, L.LOCATION_ID, L.STREET_ADDRESS
...
SELECT E.EMPLOYEE_ID                                                      AS 사원번호
     ...
     , M.MANAGER_ID                                                       AS 매니저번호
     , CONCAT(CONCAT(M.FIRST_NAME, ' '), M.LAST_NAME)                     AS 매너저명
...
SELECT ME.EMPLOYEE_ID                                                     AS 사원번호
     , CONCAT(CONCAT(ME.FIRST_NAME, ' '), ME.LAST_NAME)                   AS 사원명
     , ME.SALARY                                                          AS 급여
     , YOU.EMPLOYEE_ID                                                    AS 너사원번호
     , CONCAT(CONCAT(YOU.FIRST_NAME, ' '), YOU.LAST_NAME)                 AS 너사원명
     , YOU.SALARY                                                         AS 너급여
